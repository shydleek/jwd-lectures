...
public class PerpendicularOxSpecificationTest {
    ...
    public void sort_shouldReturnSortedList() {
        Specification<Plain> perpOxSpec = new PerpendicularOxSpecification(recorder);
        List<Plain> actualPlains = repo.findBySpecification(perpOxSpec);

        Assert.assertEquals(perpOxPlain, actualPlains.getFirst());
    }
}